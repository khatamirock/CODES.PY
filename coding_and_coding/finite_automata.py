class NFA:
    ...
    def construct_nfa_from_file(self, lines):
        self.num_states = int(lines[0]) # the total number of states......
        # assingning to the class's variable
        self.init_states()
        self.symbols = list(lines[1].strip())

        accepting_states_line = lines[2].split(" ")  # the third line of the text file is the accenpting state
        print(accepting_states_line,end='acceptance bro\n\n')
        ...
        self.startState = int(lines[3])  # the starting state recovery from text
    ...
    def convert_from_nfa(self, nfa):
        ...
        for transition in nfa.transition_functions:
            ...
            if (starting_state, transition_symbol) in nfa_transition_dict:
               ...
               # nfa_transition_dict2[{starting_state, transition_symbol}].append(ending_state)

            else:
                nfa_transition_dict[(starting_state, transition_symbol)] = [ending_state]
             #   nfa_transition_dict2[{starting_state, transition_symbol}] = [ending_state]
